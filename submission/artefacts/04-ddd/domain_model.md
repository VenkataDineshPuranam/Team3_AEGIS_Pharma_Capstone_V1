...
### Workflow D invariant extension — ADDITIONAL, OPTIONAL SCOPE, not one of the three mandated workflows

**DECISION**: `case/INTEGRATED_CASE.md` §3 permits ("Participants may conclude that a workflow... is unjustified, but must prove the decision") and, conversely, does not forbid additional bounded-context coverage beyond the three `case/INTEGRATED_CASE.md` §4 mandates. `CLAUDE.md`'s guardrails already name **"clinical eligibility"** as a prohibited autonomous action even though no clinical workflow is mandated — this extension gives that already-named boundary a concrete implementation, covering the 7 in-scope-but-previously-`out_of_scope` D03 injects (INJ-013/014/015/016/017/019/020; INJ-018 was already addressed separately as a cross-cutting exception). **This does not change RUB-08 scoring**, which evaluates the three mandated workflows specifically (`AEGIS_PROJECT_PLAN_FINAL.md` §12) — see `submission/artefacts/WORKFLOW_D_CLINICAL_TRIAL_CONTEXT.md` for the full scope statement.

| ID | Statement | Aggregate/context | Source rule or case fact | Human owner | Failure risk | Audit evidence required |
|---|---|---|---|---|---|---|
| INV-11 | A `ClinicalTrialContextBundle` must never carry a field that states or implies subject eligibility (`eligible`/`ineligible`); disagreeing eligibility-threshold sources (central/local/EDC-rule ULN) must be surfaced as a contradiction, never resolved | ClinicalTrialContextBundle | `CLAUDE.md` guardrails ("clinical eligibility" named prohibited); INJ-014; `eligibility_evidence.csv` (ALT=58 vs central_uln=40, local_uln=60, edc_rule_uln=40 — three disagreeing thresholds) | Principal Investigator / Medical Monitor | Prohibited-action violation (autonomous eligibility determination) | Contract schema has no eligibility-decision field; negative test |
| INV-12 | A `ClinicalTrialContextBundle` must never confirm, deny, or imply a subject's treatment-arm/blinding status; any evidence suggesting potential unblinding must be surfaced as a flagged risk requiring human review | ClinicalTrialContextBundle | INJ-016; `support_tickets.csv` (SUP-41: "Kit pattern suggests active arm; screenshot attached", `visibility=site_and_vendor`) | Unblinding Committee / Medical Monitor | Trial-integrity breach; regulatory reportable event | `unblinding_risk_flags` field, never an arm-assignment field |
| INV-13 | A dual-reviewer endpoint-adjudication disagreement must be surfaced as a contradiction, never resolved by the system to a single conclusion | ClinicalTrialContextBundle | INJ-019; `endpoint_packets.csv` (EP-71, `review_status=conflict`); `imaging_reviews.csv` (R1=responder, R2=non_responder) | Independent Endpoint Adjudication Committee | Wrong endpoint conclusion reaches the trial database | Contradiction entry; `required_reviews` populated |

| ID | "When X then Y must happen" | Triggering domain event | Bounded context | Accountable human owner | Source rule/fact | Audit evidence required |
|---|---|---|---|---|---|---|
| POL-07 | When a trial site's approved protocol version differs from the global-current version, then both must be surfaced with their effective dates, never silently defaulted to one | Protocol-applicability check | ClinicalTrialContextBundle | Regulatory/Clinical Operations | INJ-013; `protocol_versions.csv` (global_current 5.0 vs `site_approvals.csv` IN-014 approved=4.1) | `protocol_conflicts` field non-empty when versions differ |

**Consent-check freshness** (INJ-017, extending ADR-006's live-state-wins-over-cache principle from `authorization.py` by analogy, not a new invariant number): `processing_events.csv` (`PE-9`, `consent_check: cached_active`) shows biomarker processing proceeded on a *cached* consent check after `consents.csv` (`C-044`) shows the consent was actually `withdrawn_biomarker` — the same class of staleness bug ADR-006 already closes for authorization, applied here to consent status.
