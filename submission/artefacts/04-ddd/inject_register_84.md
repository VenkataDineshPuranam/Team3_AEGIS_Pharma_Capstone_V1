...
**Phase 6 (P6 TEVV) addendum**: the 60/12/12 count itself is unchanged by P6 — the eval harness (`submission/evaluation/`) targets injects that were already `addressed` from earlier phases (the three core workflows plus the D09/D10/D11/D12/D13 cross-cutting injects design-mapped in artefacts 16–21), so no additional inject crossed from `in_scope_open` to `addressed` this phase. What changed is evidentiary strength, not count: 21 of the `addressed` injects now also have a reproducible, machine-checked pass/fail result (`submission/evaluation/reports/final_evaluation_report.md`), not just a design-document citation — and 3 of those runs execute the unmodified `starter/legacy_pharma.py` brownfield baseline to confirm, by execution rather than assertion, that the pre-engagement code exhibits exactly the defects artefacts 16 and this register describe. The 12 `in_scope_open` injects (INJ-026, 030, 034, 046, 047, 049, 050, 052, 053, 057, 083, 084) were checked again and remain genuinely open — none is exercised by any P6 suite — and are correctly reported `NOT_RUN` in the new mirror (`submission/evaluation/inject_test_coverage.csv`), not silently marked passing.

## Risks, assumptions and unresolved gaps

| ID | Type | Description | Impact | Owner | Due / trigger | Status |
|---|---|---|---|---|---|---|
| R-001 | Risk | The `submission/evaluation` inject-test-coverage mirror now exists (`submission/evaluation/inject_test_coverage.csv`, built P6) and correctly shows `PASS` for 60 injects, `OUT_OF_SCOPE` for 12, `NOT_RUN` for the 12 still-open ones — the remaining risk is narrower: those 12 (D05/D07/D08/D13 items carried to P7) must still get real treatment before defence, not just a `NOT_RUN` label | Could under-deliver on defence element 6 (unit/term/temporal/identity conflicts) if not closed by P7 | FDE2 | P7 Ops + clean-room | Open (narrowed from Phase 3) |
